...
- 修复 AstrBot 4.27.4 在停用插件后更新时保留旧模块缓存，导致控制台版本已更新、实际帮助和命令仍是旧版的问题。
- 命令更新通过维护状态阻止玩法请求并停止后台任务，保持插件注册为启用状态，让原生重载完整清理旧模块。失败恢复也采用完整重载。
- 更新成功前校验插件类已重新导入、实际帮助标题与版本号一致；加载时记录真实帮助标题，避免仅凭 metadata 判断更新成功。
- 在网页更新时保持插件启用；若曾停用后更新，启用后再点击一次“重载”以清理旧缓存。无需重启整个 AstrBot。

### v3.1.5 管理员更新与持久配置（2026-09-17）

- 新增 `修仙更新`、`修仙更新状态`，仅 AstrBot 管理员或 `ACCESS_CONTROL.UPDATE_ADMINS` 中的用户可执行；Boss、商店管理员不自动获得更新权限。
- 更新命令仅更新本插件、仅使用本 fork 的仓库安装源；先等待当前命令结束，暂停修仙插件，备份代码、配置和 SQLite，再调用 AstrBot 原生更新并重新启用。执行期间不要同时点击网页更新。
- 更新任务跨插件重载保留，阻止重复触发；下载或加载失败时尝试恢复旧代码。若数据库结构版本已变化，停止自动恢复并保留备份供人工处理，绝不自动覆盖玩家数据库。
- 自定义游戏配置优先从 `data/plugin_data/astrbot_plugin_monixiuxian2/config/` 加载；未覆盖的文件跟随仓库默认值。历练、悬赏及其路线元数据也使用该路径。
- 默认配置不再写入插件代码目录。玩家数据库结构仍为 v20。
...
| 指令 | 权限与行为 |
| --- | --- |
| `生成Boss` | `ACCESS_CONTROL.BOSS_ADMINS`，已有存活 Boss 时不重复生成 |
| `修仙更新` | AstrBot 管理员或 `ACCESS_CONTROL.UPDATE_ADMINS`，备份后更新本 fork 默认分支并重载 |
| `修仙更新状态` | 同上，显示当前版本和最近一次命令更新结果 |
...
| 指令 | 说明 |
|------|------|
| `开始历练 <路线名>` | 共 14 条路线；从巡山问道到空间裂缝按境界逐步解锁 |
| `历练状态` | 查看当前路线进度与剩余时间 |
| `完成历练` | 领取路线奖励与奇遇掉落 |
...
插件自带以下默认配置，可按需在插件数据目录中提供同名自定义文件：
...
自定义配置放在 `data/plugin_data/astrbot_plugin_monixiuxian2/config/`，文件名与默认配置一致，按**整个文件**覆盖。可将仓库 `config/` 中需要修改的文件复制到此处再修改；宗门、Boss、秘境默认配置在 `data/default_configs.py` 中。自定义文件不会被网页更新替换；没有自定义的文件继续加载仓库默认值，因此可以收到后续新增的配方和数值调整。删除某个自定义文件并重载即可恢复该文件的仓库默认值。

**注意**：修改后在 AstrBot 控制台重载本插件生效。自定义 JSON 无法解析或顶层类型错误时会拒绝加载，避免静默丢弃设置。
...
从原作者市场版本迁移时，除修改插件 `metadata.yaml` 的 `repo` 并重载外，还需将 AstrBot 保存的插件安装源切换为仓库安装源（`repository`），绑定本 fork。仅修改 `repo` 不会覆盖已有市场绑定。

**首次从 v3.1.4 或更早版本升级前**，先备份插件目录的 `config/`，将自行修改的 JSON 文件移入 `data/plugin_data/astrbot_plugin_monixiuxian2/config/`。原生更新会先删除旧代码目录，新版本无法在启动后找回已删除的配置。只迁移有自定义改动的文件，避免把未修改的默认配方也固定为覆盖文件。完成这次迁移后，后续网页更新可直接保留外部自定义配置、玩家数据库和控制台插件设置。

管理员更新入口调用 AstrBot 内部插件管理器，已在 AstrBot 4.27.4 验证；不支持该入口的版本会提示使用控制台。命令不会改变安装源，也不会操作其他插件或更新 AstrBot 本身。
